...
onmessage = function(event) {
    ...
    const event_message = event_data["message"];

    if (event_message === "instantiate_metadata") {
        const event_metadata = event_data["metadata"];
        const enchantments_metadata = event_metadata["enchantments"];
        const item_namepace2style = event_metadata["items"];
        ...
        Object.freeze(ITEM_NAMESPACES);
    } else if (event_message === "calculate_optimal") {
        const item_entries = event_data["item_entries"];
        const item_objs = generateEnchantedItems(item_entries);
        const cheapest_work2item = cheapestItemsFromList(item_objs);
        postMessage({
            message: "optimal_searched",
            work2item: cheapest_work2item
        });
    }
};
...
function generateEnchantment(enchantment) {
    ...
    return enchantment_obj;
}

function generateEnchantments(enchantments) {
    const enchantment_count = enchantments.length;

    var enchantment_objs = new Array(enchantment_count);
    enchantments.forEach((enchantment, enchantment_index) => {
        const enchantment_obj = generateEnchantment(enchantment);
        enchantment_objs[enchantment_index] = enchantment_obj;
    });

    const enchantments_objs = new Enchantments(enchantment_objs);
    return enchantments_objs;
}

function generateEnchantedItem(item_entry) {
    const item_namespace = item_entry[0];
    const enchantments = item_entry[1];
    const prior_work = item_entry[2];

    const enchantments_obj = generateEnchantments(enchantments);
    const enchanted_item_obj = new EnchantedItem(item_namespace, enchantments_obj, prior_work);
    return enchanted_item_obj;
}

function generateEnchantedItems(item_entries) {
    const item_count = item_entries.length;
    var enchanted_item_objs = new Array(item_count);

    item_entries.forEach((item_entry, item_index) => {
        const enchanted_item_obj = generateEnchantedItem(item_entry);
        enchanted_item_objs[item_index] = enchanted_item_obj;
    });
